...
// CometenIRLAlerts - local OBS/admin control v1.3
...
public class CPHInline
{
    ...
    private bool StartIrl()
    {
        ...
        catch (Exception ex)
        {
            ...
            SendChat(L("IRL: OBS klarte ikke å starte streamen.", "IRL: OBS could not start the stream."));
            return false;
        }
    ...
    private bool GoLiveScene()
    {
        ...
        CPH.SetGlobalVar(VarIrlMode, true, true);
        SetArmed(true);
        SendChat(L("IRL: BELABOX SRT - watchdog aktiv.", "IRL: BELABOX SRT - watchdog armed."));
        return true;
    }
    ...
    private bool BackToSrt()
    {
        ...
        CPH.SetGlobalVar(VarIrlMode, true, true);
        SetArmed(true);
        SendChat(L("IRL: tilbake på BELABOX SRT - watchdog aktiv.", "IRL: back on BELABOX SRT - watchdog armed."));
        return true;
    }
    ...
    private bool StopIrl()
    {
        ...
        if (CPH.ObsIsStreaming(ObsConnection))
        {
            ...
            catch (Exception ex)
            {
                ...
                SendChat(L("IRL: OBS klarte ikke å stoppe streamen.", "IRL: OBS could not stop the stream."));
                return false;
            }
        ...
        CPH.SetGlobalVar(VarIrlMode, false, true);
        SendChat(L("IRL: stream stoppet - watchdog av.", "IRL: stream stopped - watchdog disarmed."));
        return true;
    }
    ...
    private bool SetPointsMode(string mode)
    {
        ...
        if (key == "on" || key == "irl") irlMode = true;
        else if (key == "off" || key == "normal") irlMode = false;
        else
        {
            SendChat(L("IRL: bruk !irlpoints on eller !irlpoints off.", "IRL: use !irlpoints on or !irlpoints off."));
            return false;
        }

        if (!SetRewardsMode(irlMode, true))
        {
            SendChat(L("IRL: Channel Points-gruppene kunne ikke oppdateres.", "IRL: Channel Points groups could not be updated."));
            return false;
        }
    ...
    private bool SetLanguage(string value)
    {
        ...
        if (string.IsNullOrWhiteSpace(key))
        {
            ...
            SendChat(current == "en" ? "IRL: language = English (EN)." : "IRL: språk = Norsk (NO).");
            return true;
        }

        string language;
        if (key == "no" || key == "nb" || key == "norsk" || key == "norwegian") language = "no";
        else if (key == "en" || key == "eng" || key == "english") language = "en";
        else
        {
            SendChat(L("IRL: ugyldig språk. Bruk !irllang no eller !irllang en.",
    ...
    private bool SwitchScene(string sceneName)
    {
        ...
        if (string.Equals(before, sceneName, StringComparison.Ordinal)) return true;
        ...
        try
        {
            ...
            while (waited < timeoutMs)
            {
                ...
                if (string.Equals(after, sceneName, StringComparison.Ordinal)) return true;
            }
            return false;
        }
        catch (Exception ex)
    ...
    {
        if (!GetBool(VarEndPending, false)) return;
        int sequence = GetInt(VarEndSequence, 0) + 1;
        CPH.SetGlobalVar(VarEndSequence, sequence, true);
        CPH.SetGlobalVar(VarEndPending, false, true);
    }
    ...
    private void SetArmed(bool armed) => CPH.SetGlobalVar(VarWatchdogArmed, armed, true);

    private bool ResolveAction(string explicitAction, string command, string rawInput, out string action, out string parameter)
    {
        action = string.Empty;
    ...
    private string FirstArg(params string[] names)
    {
        ...
        {
            string value;
            if (CPH.TryGetArg(name, out value) && !string.IsNullOrWhiteSpace(value)) return value.Trim();
        }
        return string.Empty;
    }
